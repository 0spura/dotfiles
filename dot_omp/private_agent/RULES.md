...
- Use the tracker MCP for work-item and PR operations, never a host CLI shortcut. Read back every tracker write, and surface a failed write as a blocker now rather than a footnote after the work is reported complete.
- When code behavior diverges from documentation, update the relevant document in the same commit: `docs/architecture.md` for data, security, or integration changes and `docs/srs.md` for requirement changes. Record a decision that changes system boundaries or public APIs as a new ADR that supersedes the old one.
