...
Query ai-memory before starting for prior architecture decisions and requirements context:

```
@ai-memory/memory_query
  namespace: architecture
  query: <project-name OR concern keywords>
```

Also query decisions namespace for relevant ADRs and grill-me outcomes:

```
@ai-memory/memory_query
  namespace: decisions
  query: <project-name OR technical decision keywords>
```

After saving, write a summary:

```
@ai-memory/memory_write_page
  namespace: architecture
  page_title: architecture/<project>/<concern>
  content: [components, boundaries, data model summary, key contracts, open decisions]
```
