...
Query ai-memory before starting for prior brainstorming, product context, or decisions on this project:

```
@ai-memory/memory_query
  namespace: product
  query: <project-name OR feature keywords>
```

After approval (Product scope), write a summary:

```
@ai-memory/memory_write_page
  namespace: product
  page_title: vision/<project>
  content: [principles, anti-goals, key decisions, approved direction]
```
