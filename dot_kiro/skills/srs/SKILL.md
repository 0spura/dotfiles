...
Query ai-memory before starting for prior requirements on this project or related decisions:

```
@ai-memory/memory_query
  namespace: requirements
  query: <project-name OR domain keywords>
```

After saving `docs/srs.md`, write a summary page:

```
@ai-memory/memory_write_page
  namespace: requirements
  page_title: srs/<project>/<domain>
  content: [domains covered, requirement count, key constraints, open questions]
```
