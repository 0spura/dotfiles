...
The implementation procedure shared by `worker`, `debug`, `perf`, and `apply-review`. The invariants live in `AGENTS.md`; this skill is the order of work.
...
6. Read the resulting code and the diff as a maintainer would: no compressed control flow, hidden invariants, mixed responsibilities, or unrelated changes.
...
Do not invent an API, configuration key, version, or behavior: return the missing decision instead. Install and update dependencies through their official command-line tooling. Do not run project-wide lint, format, or test suites, and never write tracker items, ai-memory pages, or other durable records: the caller owns integration, tracker writes, memory writes, and user communication.
