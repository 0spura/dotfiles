...
Example: `RF-CHK.2: a check-in is confirmed within 200ms of the tap` is a requirement. `The check-in opens a bottom sheet with a confirm button` is design, so it lives in `docs/design/`.
