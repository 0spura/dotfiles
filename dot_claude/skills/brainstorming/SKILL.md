...
Use when exploring how something should work before building it. The goal is to converge on an approved design, not to produce a large document.
...
3. If the user pointed to a direction, explore it rather than offering alternatives for completeness. Present 2 to 3 options only when the decision is genuinely open. For each: what it is, when it works well, main tradeoff, risk. Lead with the recommended option when there is signal.
...
5. Get approval and revise if needed. For Product scope, save to `docs/product/vision.md`.
