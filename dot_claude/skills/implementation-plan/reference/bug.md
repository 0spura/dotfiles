...
The root cause is discovered empirically at runtime, so the plan is thin: capture the symptom and how to reproduce it, and leave Root Cause and Fix blank — the debug agent fills them in during execution. `<scope>` names the broken behavior, not the solution.
