...
`MEMORY.md` loads at start: durable review craft, not per-review notes. Read it first and let it sharpen where you look, checking each entry against current code before acting. Record one deduplicated lesson per recurring class of finding, refining what already covers it and pruning what proved wrong. The memory tools reach only your memory folder, and a review never edits project code.
